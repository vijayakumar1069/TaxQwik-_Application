...
const userController = async (req, res) => {
  const generateTableHTML = (formData, returnCount) => {    
    console.log(returnCount, formData);
    let grand = 0;
    let html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <div style="background-color: #f0f0f0; padding: 20px; border-radius: 8px;">
                <h1 style="text-align: center; color: #333;">${formData.name}</h1>
...
`;
    ...
    returnCount.forEach((data, index) => {
        const trueData = Object.entries(data).filter(([key, value]) => value === true);
        const total = Object.values(data).reduce((acc, curr) => (typeof curr === "number" ? acc + curr : acc), 0);
        grand += total;
        html += `
            <div style="background-color: #fff; padding: 20px; margin-top: 20px; border: 1px solid #ccc; border-radius: 8px;">
                <h3 style="color: #333; margin-bottom: 10px;">Tax-${index + 1}</h3>
    ...
    <div style="background-color: #fff; padding: 20px; margin-top: 20px; border: 1px solid #ccc; border-radius: 8px;">
        ...
        <div style="margin-bottom: 10px;">
            ...
            <h3 style="color: #31363F; float: right;">$ ${(grand * 0.13).toFixed(2)}</h3>
        </div>
        <hr style="border-top: 1px solid #ccc; margin: 20px 0;">
        <div>
            <h2 style="display: inline-block; color: #333;">Grand Total :</h2>
            <h2 style="color: #124076; float: right;">$ ${(grand + Number(grand * 0.13)).toFixed(2)}</h2>
        </div>
    </div>
</div>
`;
  ...
  // Generate HTML table
  const tableHTML = generateTableHTML(req.body.formData, req.body.returncount);
